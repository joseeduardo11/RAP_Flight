CLASS zfe_data_generator_jp DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zfe_data_generator_jp IMPLEMENTATION.
  METHOD if_oo_adt_classrun~main.
    DELETE FROM ('zfe_atrav').
    DELETE FROM ('zfe_abook').
    DELETE FROM ('zfe_astat').
    DELETE FROM ('zfe_acarr').
    DELETE FROM ('zfe_aconn').
    DELETE FROM ('zfe_aflig').
    COMMIT WORK.
    INSERT ('zfe_atrav') FROM (
        SELECT
          FROM /dmo/travel
          FIELDS
            uuid(  )      AS travel_uuid           ,
...
*   We overwrite the existing airlines with ours
    UPDATE ('zfe_abook') SET carrier_id = 'SW' WHERE carrier_id NOT IN ( 'AA', 'AZ', 'LH', 'JL' ).
    UPDATE ('zfe_abook') SET carrier_id = 'GA' WHERE carrier_id = 'AA'.
    UPDATE ('zfe_abook') SET carrier_id = 'FA' WHERE carrier_id = 'AZ'.
    UPDATE ('zfe_abook') SET carrier_id = 'EA' WHERE carrier_id = 'LH'.
    UPDATE ('zfe_abook') SET carrier_id = 'OC' WHERE carrier_id = 'JL'.
    COMMIT WORK.
...
*   insert the new table entries
    INSERT ('zfe_astat') FROM TABLE @status_itab.
    COMMIT WORK.
...
*   insert the new table entries
    INSERT ('zfe_acarr') FROM TABLE @carrier_itab.
    COMMIT WORK.

    " insert connection demo data
    INSERT ('zfe_aconn') FROM (
        SELECT
          FROM /dmo/connection
          FIELDS
            carrier_id,
...
*   We overwrite the existing airlines with ours
    UPDATE ('zfe_aconn') SET carrier_id = 'SW' WHERE carrier_id NOT IN ( 'AA', 'AZ', 'LH', 'JL' ).
    UPDATE ('zfe_aconn') SET carrier_id = 'GA' WHERE carrier_id = 'AA'.
    UPDATE ('zfe_aconn') SET carrier_id = 'FA' WHERE carrier_id = 'AZ'.
    UPDATE ('zfe_aconn') SET carrier_id = 'EA' WHERE carrier_id = 'LH'.
    UPDATE ('zfe_aconn') SET carrier_id = 'OC' WHERE carrier_id = 'JL'.
    COMMIT WORK.

    " insert flight demo data
    INSERT ('zfe_aflig') FROM (
...
*   We overwrite the existing airlines with ours
    UPDATE ('zfe_aflig') SET carrier_id = 'SW' WHERE carrier_id NOT IN ( 'AA', 'AZ', 'LH', 'JL' ).
    UPDATE ('zfe_aflig') SET carrier_id = 'GA' WHERE carrier_id = 'AA'.
    UPDATE ('zfe_aflig') SET carrier_id = 'FA' WHERE carrier_id = 'AZ'.
    UPDATE ('zfe_aflig') SET carrier_id = 'EA' WHERE carrier_id = 'LH'.
    UPDATE ('zfe_aflig') SET carrier_id = 'OC' WHERE carrier_id = 'JL'.
    COMMIT WORK.

*   For simplicity reasons we only use one currency (EUR)
    UPDATE ('zfe_abook') SET flight_price = division( flight_price, 130, 2 )  WHERE currency_code = 'JPY'.
    UPDATE ('zfe_atrav') SET booking_fee = division( booking_fee, 130, 2 )  WHERE currency_code = 'JPY'.
    UPDATE ('zfe_atrav') SET total_price = division( total_price, 130, 2 )  WHERE currency_code = 'JPY'.
    UPDATE ('zfe_abook') SET flight_price = division( flight_price, 2, 2 )  WHERE currency_code = 'SGD'.
    UPDATE ('zfe_atrav') SET booking_fee = division( booking_fee, 2, 2 )  WHERE currency_code = 'SGD'.
    UPDATE ('zfe_atrav') SET total_price = division( total_price, 2, 2 )  WHERE currency_code = 'SGD'.
    UPDATE ('zfe_atrav') SET currency_code = 'EUR'.
    UPDATE ('zfe_abook') SET currency_code = 'EUR'.
  ENDMETHOD.
